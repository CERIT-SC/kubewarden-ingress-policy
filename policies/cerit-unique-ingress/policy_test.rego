...
test_same_host_rejected if {
	res := violation with input as review_same_host with data.inventory as inventory
	count(res) == 1
}
...
test_wildcard_same_rejected if {
	res := violation with input as review_wildcard_same with data.inventory as inventory_wildcard
	count(res) == 1
}
...
test_wildcard_overlap_rejected if {
	res := violation with input as review_wildcard_overlap with data.inventory as inventory
	count(res) == 1
}
...
test_wildcard_sub_rejected if {
	res := violation with input as review_wildcard_sub with data.inventory as inventory_wildcard_sub
	count(res) == 1
}
...
test_wildcard_host_allowed if {
	res := violation with input as review_wildcard_host with data.inventory as inventory_wildcard
	count(res) == 0
}
...
test_wildcard_diff_allowed if {
	res := violation with input as review_wildcard_diff with data.inventory as inventory_wildcard_diff
	count(res) == 0
}
...
test_same_host_diff_path_allowed if {
	res := violation with input as review_same_host_diff_path with data.inventory as inventory
	count(res) == 0
}
...
test_same_host_with_sub_allowed if {
	res := violation with input as review_same_host_with_sub with data.inventory as inventory_exact
	count(res) == 0
}
...
test_diff_host_allowed if {
	res := violation with input as review_diff_host with data.inventory as inventory_other
	count(res) == 0
}
...
test_diff_host_same_path_allowed if {
	res := violation with input as review_diff_host_same_path with data.inventory as inventory_api
	count(res) == 0
}
