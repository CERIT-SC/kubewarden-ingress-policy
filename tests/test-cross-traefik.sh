...
INGRESSROUTE_TEMPLATE=ingressroute-template.yaml
PASS=0; FAIL=0

apply_resource() {               # $1=kind $2=ns $3=host $4=path (path="-" for Gateway/IngressRoute)
...
clean_all() {
  ...
  delete_resource ir "$NS_A"
  delete_resource ir "$NS_B"
}
...
for combo_a in "${KINDS[@]}"; do
  ...
  for combo_b in "${KINDS[@]}"; do
    ...
    for scenario in "${SCENARIOS[@]}"; do
      ...
      # Use "-" for Gateway/IngressRoute path, otherwise use the scenario path
